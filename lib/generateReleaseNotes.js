...
const getLatestTag = async (Github, owner, repo) => {
  console.log(`Getting the latest tag for ${owner}/${repo}`);
  ...
  console.log(`Found tag ${name} with hash ${sha}`);
  ...
  console.log(`${name} is from ${date}`);

  return {name, date};
};

const getClosedIssues = async (Github, owner, repo, since) => {
  console.log(`Fetching all closed issues for ${owner}/${repo} since ${since}`);
...
const generateReleaseNotes = async (Github, Handlebars, fs, owner, repo, releaseName, releaseTag) => {
  console.log(`Creating ${releaseName} release notes for ${owner}/${repo} at ${releaseTag}`);
